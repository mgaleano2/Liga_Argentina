...
def scrape_stats(codigo_posicion: str) -> pd.DataFrame:
    ...
    while True:
        url = (
            ...
            f"?limit=100&offset={offset}&accumulation=total&fields={CAMPOS}"
            f"&filters=position.in.{codigo_posicion}"
        )
...
def main():
    ...
    print("Liga Profesional Argentina 2026 · top 20 detallado por posición (mín. 10 PJ)")
    for pos_en, pos_es in POSICIONES.items():
        tabla = construir_tabla(pos_en)
        print(f"\n=== TOP 20 {pos_es.upper()} ===")
        print(tabla.to_string(index=False))
        salida = os.path.join(OUT_DIR, f"top20_detallado_{pos_es}.csv")
